...
static void utc_drmclient_drm_process_request_func_36(void)
{
	...
	drm_register_setas_resp_data_s resp_info;
	memset(&resp_info, 0x0, sizeof(drm_register_setas_resp_data_s));
	...
	ret = drm_process_request(request_type, (void *) &setas_info, (void *) &resp_info);
	if (DRM_RETURN_SUCCESS == ret) {
		tet_printf("ret = 0x%x SUCCESS", ret);
		tet_result(TET_PASS);
	} else {
