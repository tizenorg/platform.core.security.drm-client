 ...
 * Library load time constructor
 */
void __drm_client_lib_unload_destructor (void) __attribute__((destructor));


/**
 * Constructor for library load time
 *
 *
 * @param 		void
 * @return		void
 ...
 * @since       0.1
 */
void __drm_client_lib_load_constructor(void)
{
	DRM_CLIENT_LOG("libdrm-client.so.0 loaded!!!");
}

/**
 * Destructor for library unload time
 *
 *
 * @param 		void
 * @return		void
 ...
 * @since       0.1
 */
void __drm_client_lib_unload_destructor(void)
{
	DRM_CLIENT_LOG("libdrm-client.so.0 unloaded!!!");

	/* Close the sockets */
	DRM_CLIENT_LOG("Closing sockets!!");
	if (sockfd >= 0) {
		close(sockfd);
		sockfd = -1;
	}
